...
public class CommandServlet extends HttpServlet {
    ...
    private static void print(HttpServletResponse resp, String s) {
        try {
    ...
    private void getTimeRange(HttpServletRequest req, HttpServletResponse resp) {
        ...
        String json = db.getTimeRangeJson(start,
                new Date(finish.getTime() + TimeUnit.DAYS.toMillis(1)));
        resp.setContentType("application/json");
        print(resp, json);
    }
        ...
        try {
            ...
            while ((line = br.readLine()) != null) {
                if (line.trim().length() > 0) {
                    ...
                    db.saveTime(UUID.randomUUID().toString(), t1, t2.getTime() - t1.getTime());
                    count++;
                }
            }
            br.close();
            print(resp, count + " entries loaded");
        } catch (IOException e) {
            throw new RuntimeException(e);
