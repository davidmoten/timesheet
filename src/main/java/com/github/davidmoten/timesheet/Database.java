...
public class Database {
    ...
    public void saveTime(String id, Date start, long durationMs) {
        ...
        // kind=db,name=schema,
        Key timesheetKey = createTimesheetKey();
    ...
    private static Key createTimesheetKey() {
        return KeyFactory.createKey("Timesheet", "Timesheet");
    }
    ...
    public String getTimesJson(int n) {
        ...
        Filter sinceFilter = new FilterPredicate("startTime", FilterOperator.GREATER_THAN_OR_EQUAL,
                new Date(t));
        Filter userFilter = new FilterPredicate("user", FilterOperator.EQUAL, user);
        Filter userAndSinceFilter = CompositeFilterOperator.and(userFilter, sinceFilter);
        Query q = new Query("Entry").setFilter(userAndSinceFilter).addSort("startTime",
    ...
    public String getTimeRangeJson(Date start, Date finish) {
        ...
        User user = getUser();
        Filter afterFilter = new FilterPredicate("startTime", FilterOperator.GREATER_THAN_OR_EQUAL,
                start);
        Filter beforeFilter = new FilterPredicate("startTime", FilterOperator.LESS_THAN, finish);
        Filter userFilter = new FilterPredicate("user", FilterOperator.EQUAL, user);
        Filter userAndTimeFilter = CompositeFilterOperator.and(userFilter, afterFilter,
                beforeFilter);
        Query q = new Query("Entry").setFilter(userAndTimeFilter).addSort("startTime",
                SortDirection.ASCENDING);
    ...
    public void deleteEntry(String id) {
        ...
        Filter idFilter = new FilterPredicate("entryId", FilterOperator.EQUAL, id);
    ...
    private static String toJson(Iterable<Entity> entities) {
        ...
        for (Entity entity : entities) {
            ...
            SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:00.000'Z'");
            df.setTimeZone(TimeZone.getTimeZone("UTC"));
            if (!first)
                s.append(",\n");

            s.append("      {\"startTime\" : \"").append(df.format(startTime)).append("\"")
                    .append(",");
            s.append("\"durationMs\" : ").append("\"").append(durationMs).append("\"").append(",");
            s.append("\"id\" : ").append("\"").append(id).append("\"").append("}");
            first = false;
        }
        s.append("\n]}");
    ...
    public void setSetting(String key, String value) {
        ...
        Filter userFilter = new FilterPredicate("user", FilterOperator.EQUAL, user);
        Filter userAndKeyFilter = CompositeFilterOperator.and(userFilter, keyFilter);
    ...
    public String getSetting(String key) {
        ...
        Filter userFilter = new FilterPredicate("user", FilterOperator.EQUAL, user);
        Filter keyFilter = new FilterPredicate("key", FilterOperator.EQUAL, key);
        Filter userKeyFilter = CompositeFilterOperator.and(userFilter, keyFilter);
